  ...
  var ckg_iwikiClass;
	...
	return {
		...
		onOk : function()
		{
			...
			var ok_callback = jQuery.proxy(function (data, heading) {
				...
				switch ( data.type || 'url' )
				{
					...
					case 'internal':
                        ...
                        data.adv.advCSSClasses = "wikilink1";
						 if(heading && oDokuWiki_FCKEditorInstance.useheading == 'y') {
							var tmp = heading;
            ...
            if (data.type == 'internal' && oDokuWiki_FCKEditorInstance.useheading == 'y') {
                jQuery.when(useHeading(wikiIdFromUrl(data.url.url)).then(function (heading) {
                    ok_callback(data, heading);
				}));
			} else {
                ok_callback(data);
			}
