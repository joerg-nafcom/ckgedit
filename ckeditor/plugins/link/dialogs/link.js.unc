...
*/
oDokuWiki_FCKEditorInstanceInLinkDialog = true;
var update_ckgeditInternalLink, update_ckgeditMediaLink;
var fckgInternalInputId, fckgMediaInputId,ckgeditIwikiIndex,ckgeditIwikiData;
  ...
  var ckg_iwikiClass;
	...
	{  
		...
		if ( !retval.type )
		{
            ...
            else if( (urlMatch = href.match( oRegex.internal_link )) || (urlMatch = href.match( internalLinkRegex )) || ( urlMatch = href.match(oRegex.internal_link_rewrite_2)) 
                     ...
                     ) 
            {
                ...
                editor.config.selectedIwikiIndex = index;                  
                iw_select.disable();                   
	...
	return {
		...
		onOk : function()
		{
			   ...
			   var other_mime_file = "";
				switch ( data.type || 'url' )
				{
					...
					case 'internal':
                        ...
                        data.adv.advCSSClasses = "wikilink1";
						 ...
						 if(internal_text && oDokuWiki_FCKEditorInstance.useheading == 'y') {
							var tmp = internal_text;
					...
					case 'interwiki':    
                        if(typeof editor.config.selectedIwikiIndex == "number") {
                            var id = this.getContentElement('info',  'iwiki_shortcut').getInputElement().$.id;
                            var select = document.getElementById(id);                           
                            select.selectedIndex = editor.config.selectedIwikiIndex;                           
                        }
					...
					case 'url':
						...
						url = ( data.url && CKEDITOR.tools.trim( data.url.url ) ) || '';
					    attributes[ 'data-cke-saved-href' ] = ( url.indexOf( '/' ) === 0 ) ? url : protocol + url;
                        url_text =  this.getValueOf( 'info', 'url_text' );
						break;
					case 'anchor':
				...
				if ( !this._.selectedElement )
				{
				 ...
				 if(navigator.userAgent.match(/(Trident|MSIE)/)  || !text) {
				...
				else
				{
                    ...
                    if(internal_text){
                        element.setText(internal_text);
                    }
                    else if(iwiki_text && iwiki_text != data.adv.advTitle) { 
                      var url =   href.replace(iwiki_text, data.adv.advTitle);                      
                      element.setText(url);                      
                    }  
           ...
           if(text) { 
              try {   
              if(media_text) {
                   text.setText(media_text);
               }                  
			   else if(internal_text) {
               	   text.setText(internal_text);
			   }
              else if(text && iwiki_text) {                   
                   text.setText(iwiki_text);
               }                                
               else if(url_text) {
		...
		onLoad : function()
		{
            ...
            var setUpIwikiSel = function() { 
               ...
               for(var i in ckgeditIwikiData) {
                  this.stack[count] =  new Option(i + ' >> ' + ckgeditIwikiData[i],i,false,false); 
       ...
       // this.getContentElement("info", 'iwiki_shortcut'). disable();  //restricts attempts to enter filenames into text box
        ...
        var update_schemes = function(proto_ref, ed) {                      
            var selectx = document.getElementById(proto_ref);   
            var schemes = ed.config.ckgProtocols;          
            var ar = schemes.split(/\n/);
            var ar_final = new Array();
            for(i=0;i<ar.length;i++) {
                if(!ar[i].match(/\w/)) continue;
                if(ar[i].match(/\#|http|https|ftp|news|m-files/)) {
                continue;              
                }          
       ...
       var proto_ref = this.getContentElement('info', 'protocol').getInputElement().$.id 
       update_schemes(proto_ref, editor);
 ...
 */
    ...
    insertLink: function(title){
        ...
        if(title && oDokuWiki_FCKEditorInstance.useheading == 'y') {
          dialog.getContentElement("info", "internal_text").setValue(title);
        }
      else {       
